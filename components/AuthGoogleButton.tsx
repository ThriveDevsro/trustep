...
const buttonClassName =
  'relative flex h-12 w-full items-center justify-center rounded-xl border border-slate-300 bg-white px-12 text-sm font-semibold text-[#0F172A] focus:outline-none focus:ring-4 focus:ring-[#2563EB]/10 disabled:cursor-not-allowed disabled:opacity-50'

export function AuthGoogleButton({ loading, onClick, label }: AuthGoogleButtonProps) {
  return (
    <button type="button" onClick={onClick} disabled={loading} className={buttonClassName}>
      {loading ? (
        <Loader2 className="h-5 w-5 animate-spin" />
      ) : (
        <svg aria-hidden="true" viewBox="0 0 24 24" className="absolute left-4 h-5 w-5">
  ...
  return (
    <button type="button" onClick={onClick} disabled={loading} className={buttonClassName}>
      {loading ? (
        ...
        <svg aria-hidden="true" viewBox="0 0 23 23" className="absolute left-4 h-5 w-5">
